...
def test_algorithm(alg, volume, geometry, sino, betas, iterations, phantom):
    ...
    fig, axes = plt.subplots(len(betas), len(iterations), figsize=(4 * len(iterations), 4 * len(betas)))

    errors = []
    # for every combination, run the reconstruction algorithm (use initial guess to minimize work)
    for i in range(0, len(betas)):
        ...
        while len(iterations) > 0 and k <= iterations[-1]:
            if k == iterations[j]:
                ...
                axes[i, j].set_title('beta = {:.2f}, iters = {}'.format(betas[i], k))
                j += 1
