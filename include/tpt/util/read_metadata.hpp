...
math::vec<D, T> stdvec_to_tomovec(std::vector<S> in) {
    assert(in.size() == D);

    auto out = math::vec<D, T>();
    for (int d = 0; d < D; ++d) {
        out[d] = in[d];
    }
...
template <dimension D, typename T, typename R>
math::vec<D, T> read_vec_as_(std::shared_ptr<cpptoml::table> parameters,
                                   std::string name) {
    return stdvec_to_tomovec<D, T>(*parameters->get_array_of<R>(name));
}

template <dimension D, typename T>
math::vec<D, T> read_vec_(std::shared_ptr<cpptoml::table> parameters,
...
read_circular_cone_beam_geometry(std::shared_ptr<cpptoml::table> parameters,
    ...
    return std::make_unique<geometry::cone_beam<T>>(
        v, projection_count, detector_size, detector_shape, source_position,
        detector_position, std::array<math::vec<3_D, T>, 2>{
                               detector_tilt_vec[0], detector_tilt_vec[1]});
}

template <typename T>
std::unique_ptr<geometry::helical_cone_beam<T>>
read_helical_cone_beam_geometry(std::shared_ptr<cpptoml::table> parameters,
    ...
    return std::make_unique<geometry::helical_cone_beam<T>>(
        v, projection_count, detector_size, detector_shape, source_position,
        detector_position, rotations,
        std::array<math::vec<3_D, T>, 2>{detector_tilt_vec[0],
                                               detector_tilt_vec[1]});
}
...
template <typename T>
std::unique_ptr<geometry::laminography<T>>
read_laminography_geometry(std::shared_ptr<cpptoml::table> parameters,
    ...
    return std::make_unique<geometry::laminography<T>>(
        v, projection_count, detector_size, detector_shape, source_position,
        detector_position, source_radius, detector_radius);
}

template <typename T>
std::unique_ptr<geometry::tomosynthesis<T>>
read_tomosynthesis_geometry(std::shared_ptr<cpptoml::table> parameters,
    ...
    return std::make_unique<geometry::tomosynthesis<T>>(
        v, projection_count, detector_size, detector_shape, source_position,
...
read_parallel_geometry(std::shared_ptr<cpptoml::table> parameters,
    ...
    return std::make_unique<geometry::parallel<D, T>>(v, angle_count);
}

template <dimension D, typename T>
std::unique_ptr<geometry::dual_axis_parallel<D, T>>
read_dual_parallel_geometry(std::shared_ptr<cpptoml::table> parameters,
    ...
    return std::make_unique<geometry::dual_axis_parallel<D, T>>(
        v, angle_count);
}

template <dimension D, typename T>
std::unique_ptr<geometry::base<D, T>>
...
template <dimension D, typename T>
volume<D, T> read_volume(std::shared_ptr<cpptoml::table> parameters, int k = -1) {
    ...
    return volume<D, T>(voxels, min_point, max_point - min_point);
}

template <dimension D, typename T>
std::unique_ptr<projections<D, T>>
read_projection_stack(std::shared_ptr<cpptoml::table> parameters,
                      geometry::base<D, T>& g, fs::path root_directory) {
	return nullptr;
    try {
        ...
        return std::make_unique<projections<D, T>>(
            std::move(tiff_stack_to_projections<D, T>(
