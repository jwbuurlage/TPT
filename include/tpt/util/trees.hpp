...
void add_to_voxel_tree(bulk::util::binary_tree<bulk::util::split>::node* voxel,
                       tree_node<T>* neutral, volume<3_D, T> v) {
    if (neutral && voxel) {
...
from_neutral_tree(tree_node<T>& splits, volume<3_D, T> v) {
...
load_partitioning(std::string filename, volume<3_D, T> v,
                  int depth = -1) {
    auto reloaded = deserialize_tree<T>(filename, depth);
    if (!reloaded) {
        std::cout << "Could not deserialize: " << filename << "\n";
        return nullptr;
    }

    auto large_voxel = from_neutral_tree<T>(*reloaded, v);
