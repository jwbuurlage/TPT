...
image<2_D, T> tiff_to_image(std::string filename) {
...
(void)filename;
    ...
    auto v = volume<2_D, T>({1, 1});
    return image<2_D, T>(v);
}

template <dimension D, typename T = default_scalar_type>
projections<D, T>
tiff_stack_to_projections(const geometry::base<D, T>& g,
...
template <typename T = default_scalar_type>
void write_png(image<2_D, T> x, std::string filename) {
    (void)x;
    (void)filename;
//    auto v = x.get_volume();
//
...
template <typename T = default_scalar_type>
void write_png(image<3_D, T> x, std::string filename) {
    write_png(slice(x, x.get_volume()[0] / 2, 0), filename + "_x");
    write_png(slice(x, x.get_volume()[1] / 2, 1), filename + "_y");
    write_png(slice(x, x.get_volume()[2] / 2, 2), filename + "_z"); 
}
