...
image<D, T> sirt(const volume<D, T>& v, const geometry::base<D, T>& g,
    ...
    auto rs = row_sums<D, T>(g, kernel);
    auto bcs = column_sums<D, T>(g, kernel);
...
template <dimension D, typename T>
image<D, T> landweber(const volume<D, T>& v, const geometry::base<D, T>& g,
                 dim::base<D, T>& kernel, const projections<D, T>& p,
                 double beta = 1.0, int iterations = 10,
                 std::function<void(image<D, T>&, int)> callback = {},
                 bool box_constraint = false, T box_min = -1, T box_max = 1) {
