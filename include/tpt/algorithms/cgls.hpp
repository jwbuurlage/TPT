...
image<D, T> cgls(const volume<D, T>& v, const geometry::base<D, T>& g,
                 dim::base<D, T>& kernel, const projections<D, T>& b,
                 int iterations = 10,
    ...
    // r0 = A^T b
    auto r = back_projection(b, g, kernel, v);
    ...
    for (int k = 0; k < iterations; ++k) {
        // (..7) t_k = A p_k
        auto t = forward_projection(p, g, kernel);
        ...
        // (4) r_k = A^T d_k
        r = back_projection(d, g, kernel, v);
 ...
 */
template <dimension D, typename T>
image<D, T> cgls2(const volume<D, T>& v, const geometry::base<D, T>& g,
                  dim::base<D, T>& kernel, const projections<D, T>& b,
                  int iterations = 10,
                  std::function<void(image<D, T>&, int)> callback = {}) {
    using namespace img;
    ...
    auto cs = column_sums<D, T>(g, kernel);
    for (auto& c : cs) {
    ...
    // From Li and Saad
    auto r = b;
    auto tr = back_projection(r, g, kernel, v);
    auto z = cs * tr;
    auto p = z;

    for (int k = 0; k < iterations; ++k) {
        auto w = forward_projection(p, g, kernel);
        auto wnorm = math::norm(w);
        auto gamma = math::dot(z, tr);
        auto alpha = gamma / (wnorm * wnorm);
        x = x + alpha * p;
        r -= alpha * w;
        tr = back_projection(r, g, kernel, v);
        z = cs * tr;
...
template <dimension D, typename T>
image<D, T> cg(const volume<D, T>& v, const geometry::base<D, T>& g,
               dim::base<D, T>& kernel, const projections<D, T>& b,
               int iterations = 10,
               std::function<void(image<D, T>&, int)> callback = {}) {
    using namespace img;

    auto y = projections<D, T>(g);
    auto r = b;
    auto p = r;

    for (int k = 0; k < iterations; ++k) {
        auto gamma = math::dot(r, r);
        auto bpp = back_projection(p, g, kernel, v);
        auto alpha = gamma / math::dot(bpp, bpp);
        y += alpha * p;
        r -= alpha * forward_projection(bpp, g, kernel);
        auto beta = math::dot(r, r) / gamma;
        p *= beta;
        p += r;

        if (callback) {
            auto x = back_projection(y, g, kernel, v);
            callback(x, k);
        }
    }
