...
image<D, T> column_action_cyclic(
    volume<D, T>& v, const geometry::base<D, T>& g,
    ...
    std::function<void(const image<D, T>&, int, const projections<D, T>&)>
        callback = {}) {
    auto x = x0.value_or(image<D, T>(v, 0));

    write_png(x, "fan_beam_initial");
    auto ax = forward_projection(x, g, kernel);

    auto r = b - ax;
    auto column = column<D, T>(g, kernel);
