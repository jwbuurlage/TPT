...
// FIXME: Should dispatch to the proper specialization instead of this
// duplication
...
template <typename T>
class parallel_geometry<3_D, T>
    : public geometry<3_D, T, parallel_line_iterator<3_D, T>> {
  public:
    using iterator_type = parallel_line_iterator<3_D, T>;

    parallel_geometry(int angle_count, int detector_count,
                      const volume<3_D>& volume)
        : geometry<3_D, T, iterator_type>(angle_count * detector_count *
                                          detector_count),
          volume_(volume) {
        auto angle_step = math::pi / angle_count;
        for (T angle = 0.0; angle < math::pi; angle += angle_step) {
        ...
        // make two dimensional detector grid
        // FIXME
        //        auto detector_step = (T)volume.y() / detector_count;
        //        for (int detector = 0; detector < detector_count; detector++)
        //        {
        //            detectors_.push_back((detector - (detector_count - 1) *
        //            0.5) *
        //                                 detector_step);
        //        }
    ...
    const volume<3_D>& get_volume() const { return volume_; }

  private:
    std::vector<T> angles_;
    std::vector<math::vec2<T>> detectors_;
    volume<3_D> volume_;
};

template <typename T>
class parallel_line_iterator<3_D, T>
    : public std::iterator<std::forward_iterator_tag, line<3_D, T>> {
  public:
    parallel_line_iterator(int i, const parallel_geometry<3_D, T>& geometry)
        : i_(i), geometry_(geometry) {}
    ...
    line<3_D, T> operator*() {
        ...
        for (auto& line_segment : lines) {
            auto intersection_point = math::intersection<T>(
                origin, end, line_segment[0], line_segment[1]);
            if (intersection_point != math::vec2<T>(0, 0)) {
                auto dist = math::distance<3_D, T>(origin, intersection_point);
                if (dist < min_distance) {
                    best_point = intersection_point;
                    min_distance = dist;
                }
  ...
  private:
    int i_;
    const parallel_geometry<3_D, T>& geometry_;
};
